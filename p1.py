...
# def p1(img, x, y, filename):
#     # second: 웰딩라인 후보영역 얻어오기
#     # TODO: 동휘한테 전달 후 후보영역 얻어오기
#     img = cv2.imread('../data/welding_data/more_test_1004/clickRoi/' + filename)
#     temp_msk = cv2.imread('../data/welding_data/more_test_1004/areaMask/' + filename)
#
#     # third: 후보영역 적당히 확장 후 마스킹
#     # 중요: 팽창 횟수가 1만큼 차이나는 2개의 마스크를 만들어 최외곽에 대한 edge를 제거하고자 한다.
#     # TODO: 적당한 팽창 횟수 찾기
#     msk1 = cv2.dilate(temp_msk, mask, iterations=4)
#     msk2 = cv2.dilate(temp_msk, mask, iterations=3)
#
#     binary = cv2.cvtColor(msk1, cv2.COLOR_RGB2GRAY)
#     msk2 = cv2.cvtColor(msk2, cv2.COLOR_RGB2GRAY)
#     contours, hierarchy = cv2.findContours(binary, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
#     max_contour = max(contours, key=cv2.contourArea)
#
#     hull = cv2.convexHull(max_contour, clockwise=True)
#     cv2.drawContours(msk1, [hull], 0, (255, 255, 255), -1)
#     dst1 = cv2.bitwise_and(img, msk1)
#
#     # x, y 최소, 최대 값으로 roi 한정
#     x_min = np.min(max_contour[:, :, 0])
#     x_max = np.max(max_contour[:, :, 0])
#     y_min = np.min(max_contour[:, :, 1])
#     y_max = np.max(max_contour[:, :, 1])
#     roi1 = dst1[y_min:y_max, x_min:x_max]
#     msk2 = msk2[y_min:y_max, x_min:x_max]
#
#     # fourth: 밝기 조절 및 영역 분할
#     # roi1_hf = hf.Homomorphic_Filter(roi1)  # 이거 전에 배경제거 해야될 거 같은데 roi 추출
#     # fifth: edge 추출 및 분석
#     dst2 = cv2.GaussianBlur(roi1, (0, 0), 1)
#     canny1 = cv2.Canny(dst2, 100, 150)
#     canny2 = cv2.bitwise_and(canny1, msk2)
#
#     # 이제 쪼개봐
#     roi_cuts = []
#     x_cut = canny2.shape[1] // 2
#     y_cut = canny2.shape[0] // 10
#     # print(canny1.shape)
#     # print(x_cut, y_cut)
#
#     y_start = 0
#     for i in range(20):
#         x_start = (x_cut + 1 if i % 2 == 1 else 0)
#         x_end = x_start + x_cut
#
#         y_start += y_cut if i != 0 and i % 2 == 0 else 0
#         y_end = y_start + y_cut
#
#         roi_cuts.append(canny2[y_start:y_end, x_start:x_end])
#
#     for j, roi_cut in enumerate(roi_cuts):
#         _, thresholded_image = cv2.threshold(roi_cut, 150, 1, cv2.THRESH_BINARY)
#         # print(thresholded_image)
#         # cv2.imshow("test", thresholded_image)
#         # cv2.waitKey(0)
#         # cv2.destroyAllWindows()
#
#         # print(filename, j, thresholded_image.sum(), sep='\t')
#         # print(f"{filename}_cuts{j}", sum(roi_cut))
#         # cv2.imshow(f"roi_cut_{j}", roi_cut)
#         # cv2.imwrite("../data/welding_data/more_test_1004/cuts/thr_" + f"{filename[:-4]}_{str(j).zfill(2)}.jpg", thresholded_image)
#         # cv2.imwrite("../data/welding_data/more_test_1004/cuts/" + f"{filename[:-4]}_{str(j).zfill(2)}.jpg", roi_cut)
#
#     # sixth: 평가
#
#     cv2.imshow("img", img)
#     cv2.imshow("temp_msk", temp_msk)
#     cv2.imshow("msk1", msk1)
#     cv2.imshow("msk2", msk2)
#     cv2.imshow("roi", roi1)
#     cv2.imshow("dst1", dst1)
#     # cv2.imshow("dst2", roi1_hf)
#     cv2.imshow("canny1", canny1)
#     cv2.imshow("canny2", canny2)
#     # cv2.imwrite("../data/welding_data/more_test_1004/weldingLineEdge/" + filename[:-4]+ "_edge" + ".jpg", canny2)
#     cv2.waitKey(0)
#     cv2.destroyAllWindows()
